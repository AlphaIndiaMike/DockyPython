		...
		if (this.areas[i]) {
			if (this.areas[i].shape && this.areas[i].shape != 'undefined') {
				...
				html+= '<area shape="' + this.areas[i].shape + '"' +
                    ...
                    ' onclick="$.prompt(write_part('+"'"+this.areas[i].ahref+"'"+','+"'"+this.areas[i].aalt+"'"+'),{ buttons: { Continue: true, Cancel: false },callback: callbackfunction })"'+
					' coords="' + coords + '"' +
					' href="#" />';
					//+' target="' + this.areas[i].atarget + '" href="#"/>';
			}
		}
	}
	//alert(html);
